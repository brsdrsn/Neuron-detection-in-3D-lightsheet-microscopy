"""
Pipeline stage: — demo / spike (not production)
===============================================
Smoke-test YOLO on one tagged tile using pretrained COCO weights (yolov8n.pt).
Shows per-slice bounding boxes and centers in Napari. Detections are not meaningful
for neurons — use visualize_on_napari.py with trained weights for real QC.

Inputs:  hard-coded tile TIFF under ~/Documents/neurons/tagged tiles/
Outputs: interactive Napari window (no files written)

Run:     python run_yolo.py
Suggested rename: demo_pretrained_yolo_napari.py
Redundant with: visualize_on_napari.py (which uses your trained model instead)
"""
...
# ---- LOAD 3D TILE (Z,Y,X) ----
tile_path = str(
    Path.home() / "Documents" / "neurons" / "tagged tiles" / "tile 2-2-0" / "Tile 2-2-0.tif"
)

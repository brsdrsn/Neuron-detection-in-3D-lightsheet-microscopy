"""
Pipeline stage: 0 — exploration (legacy)
==========================================
Quick Napari viewer for a fused lightsheet Zarr chunk. Loads the raw multiscale
array with Dask, squeezes channel/time dims to (Z, Y, X), rechunks into tiles,
and opens one tile in Napari.

This predates the napari-large-tiler plugin; tiling/preprocessing here duplicates
napari_large_tiler._tiling.process_data() and tile_array(). Prefer the plugin or
rebuild_tiles.py / infer_chunk_global_coords.py for production work.

Inputs:  hard-coded Zarr path under ~/Documents/neurons/
Outputs: interactive Napari window (no files written)

Run:     python main.py

Also imported by: cellpose_demo.py (process_data, tile helpers)
"""
import napari

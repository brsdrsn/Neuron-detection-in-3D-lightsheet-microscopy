...
| 2 — Dataset | `build_yolo_dataset.py` | Masks → YOLO labels; export PNG train/val set |
| 3 — Training | `train_yolo.py` | Fine-tune YOLOv8 on exported dataset |
| 4 — Inference | `infer_chunk_global_coords.py` | Detect neurons across a full Zarr chunk |
| 4b — QC | `qc_yolo_detections_tile_napari.py` | Visual check on a single tile |
| 5 — View results | `view_neuron_centers_on_chunk_napari.py` | Overlay points on full chunk in Napari |
| 6 — Analysis | `plot_neuron_density_mip.py` | Top-down XY density map from CSV |
...
Volumes are multiscale Zarr stores from fused lightsheet acquisitions

Scripts accept the full path including the trailing `\1`.
...
The structure nescessary for the training:
...
Napari/outputs/chunk_<id>/global_coords/
  ...
  neuron_centers_chunk_<id>_all_tiles_raw_axes.csv # Napari points (raw Zarr axes)  (Use this if directly viewing within napari)
```

---

## Browse and tile volumes with napari-large-tiler
...
2. Set **tile size** in Z, Y, X (default 256³ in the UI; For the training I used 512³ — keep these consistent).
...
5. Writes `~/Documents/neurons/yolo_tagged_tiles_v3/data.yaml`. (Adjust path accordingly)
---
...
| Data | `~/Documents/neurons/yolo_tagged_tiles_v3/data.yaml` | set this to your desired zarr file path|
...
2. Divide into tiles (same grid as labeling).
...
4. Export CSV (with confidence + tile metadata) and Napari-compatible point CSVs.
...
To load points from Napari's GUI instead: **Layer → Add Points → Open file** and choose `neuron_centers_*_raw_axes.csv`, or just drag the file on to the window.
...
If you are not performing model training and you only need to detect neurons in a new chunk:
...
├── qc_yolo_detections_tile_napari.py # Process only one tile
